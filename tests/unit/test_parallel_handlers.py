...
async def test_cc_cancels_channel_scoped_executions_when_not_in_thread() -> None:
    ...
    assert "Cancelled 1 job(s) and 3 active execution(s)." in client.chat_postMessage.await_args.kwargs[
        "text"
    ]
...
async def test_cc_cancels_thread_scoped_executions_when_in_thread() -> None:
    ...
    deps.executor.cancel_all.assert_not_awaited()
    assert "Cancelled 1 job(s) and 2 active execution(s)." in client.chat_postMessage.await_args.kwargs[
        "text"
    ]
