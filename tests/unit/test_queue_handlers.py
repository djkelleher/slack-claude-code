...
async def test_execute_queue_item_passes_plan_mode_directive_to_router():
    ...
    async def _fake_execute_for_session(**kwargs):
        seen_sessions.append(kwargs["session"])
        captured_plan_modes.append(kwargs["plan_mode_directive"])
        return SimpleNamespace(
            backend="codex",
            result=SimpleNamespace(
                success=True,
    ...
    assert result == "completed"
    assert seen_sessions[0].sandbox_mode == "read-only"
    assert captured_plan_modes[0] is not None
    assert captured_plan_modes[0].strategy == "splan"
    assert captured_plan_modes[0].models == ("claude-sonnet-4-6-high", "gpt-5.4-high")
...
async def test_qc_delete_deletes_entire_queue_scope():
    ...
    client = SimpleNamespace(chat_postMessage=AsyncMock())
    with patch(
        "src.handlers.claude.queue.TaskManager.cancel",
        new=AsyncMock(return_value=False),
    ) as mock_cancel:
        await handler(
            ack=AsyncMock(),
...
@pytest.mark.asyncio
async def test_q_structured_auto_directives_add_metadata_and_finish_flag():
    """Structured `/q` should persist auto metadata and auto-finish scope flag."""
    app = _FakeApp()
    deps = SimpleNamespace(
        db=SimpleNamespace(
            get_or_create_session=AsyncMock(return_value=Session(id=1, working_directory="/repo")),
    ...
    assert len(queued_entries[0]) == 5
    assert queued_entries[0][4]["auto_each"] is True
    deps.db.set_queue_auto_finish_pending.assert_awaited_once_with("C123", None, True)
    assert (
