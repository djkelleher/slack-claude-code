...
class TestCommandRouter:
    ...
    @pytest.mark.asyncio
    async def test_claude_plan_approval_runs_follow_up_turn_after_approval(self):
        """Claude plan approvals should trigger implementation follow-up in bypass mode."""
        deps = SimpleNamespace(
            db=SimpleNamespace(
                update_session_claude_id=AsyncMock(),
                update_session_codex_id=AsyncMock(),
        ...
        deps.executor.execute = AsyncMock(side_effect=[first_turn, second_turn])
        session = Session(
            id=26,
            model="opus",
            working_directory="/tmp",
        ...
        with patch(
            ...
            routed = await execute_for_session(
                ...
                channel_id="C123",
                thread_ts=None,
                execution_id="exec-claude-plan",
                slack_client=SimpleNamespace(),
                user_id="U123",
            )

        request_approval.assert_awaited_once()
        deps.db.update_session_mode.assert_awaited_once_with(
            "C123", None, config.DEFAULT_BYPASS_MODE
        )
        assert deps.executor.execute.await_count == 2
        second_kwargs = deps.executor.execute.await_args_list[1].kwargs
        assert (
            second_kwargs["prompt"]
            == "Plan approved. Please proceed with the implementation."
        )
        assert second_kwargs["permission_mode"] == config.DEFAULT_BYPASS_MODE
        assert routed.result.success is True
        assert routed.result.output == "Implementation complete."

    @pytest.mark.asyncio
    async def test_claude_auto_approves_permissions_for_queue_execution(self):
        """Auto-approve mode should bypass Slack permission UI for Claude prompts."""
        deps = SimpleNamespace(
            db=SimpleNamespace(
                update_session_claude_id=AsyncMock(),
                update_session_codex_id=AsyncMock(),
        ...
        async def _fake_claude_execute(**kwargs):
            approved = await kwargs["on_tool_permission_request"](
                "Bash", {"command": "ls"}
            )
        ...
        with patch(
            ...
            routed = await execute_for_session(
                ...
                channel_id="C123",
                thread_ts=None,
                execution_id="exec-9-auto-claude-approval",
                slack_client=SimpleNamespace(),
                user_id="U123",
                auto_approve_permissions=True,
            )
    ...
    @pytest.mark.asyncio
    async def test_claude_approval_resume_can_swap_streaming_callback(self):
        """Claude approvals should allow streaming to move to a new Slack message."""
        deps = SimpleNamespace(
            db=SimpleNamespace(
                update_session_claude_id=AsyncMock(),
                update_session_codex_id=AsyncMock(),
