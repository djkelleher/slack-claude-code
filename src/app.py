...
from src.handlers.slash_command_router import parse_slash_command_text
...
def _extract_single_prompt_mode_directive(prompt: str) -> tuple[str, Optional[str]]:
    ...
    if end_indices:
        last_non_empty_index = max(idx for idx, line in enumerate(remaining_lines) if line.strip())
...
def _parse_typed_diff_command(raw_text: str) -> tuple[str, str] | None:
    """Normalize typed `/diff` message syntax into slash-router inputs."""
    stripped = (raw_text or "").strip()
    if not stripped:
        return None

    compact_match = _COMPACT_DIFF_COMMAND_RE.match(stripped)
    if compact_match:
        return "/diff", compact_match.group("selection")

    parsed = parse_slash_command_text(stripped)
    if parsed and parsed.name == "/diff":
...
async def _post_message_processing_error(
    ...
    await client.chat_postMessage(
        ...
        blocks=error_message("Unexpected error while processing this message.\n" f"{details}"),
    )
...
async def _route_claude_message_to_active_execution_or_queue(
    ...
    await deps.db.update_command_status(
        ...
        output=(
            (
                f"Active Claude execution detected (steer failed: {steer_error}). "
                if steer_error
                else "Active Claude execution detected. "
            )
...
async def main():
    ...
    async def handle_message(event, client, logger):
        ...
        typed_diff_command = _parse_typed_diff_command(prompt)
        if (
            typed_diff_command is not None
            and deps.slash_command_router is not None
            and deps.slash_command_router.has_command(typed_diff_command[0])
        ):
            logger.info("Detected typed /diff message; routing via slash command handler")
            await deps.slash_command_router.dispatch(
                command_name=typed_diff_command[0],
                command_text=typed_diff_command[1],
                channel_id=channel_id,
                thread_ts=thread_ts,
                user_id=user_id,
                client=client,
        ...
        try:
            ...
            if runtime_mode_directive:
                ...
                runtime_plan_mode = runtime_resolution.plan_mode
            ...
            await execute_prompt_with_runtime(
                ...
                plan_mode_directive=runtime_plan_mode,
            )
        except asyncio.CancelledError:
            return
        except Exception as e:
