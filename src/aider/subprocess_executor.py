"""Subprocess executor for Aider AI coding assistant.

Spawns ``aider`` in non-interactive mode using ``--message`` for the prompt
and ``--yes`` for auto-confirmation. Output is streamed line-by-line from
stdout and parsed into StreamMessage objects.
"""
...
from src.aider.streaming import StreamParser
...
class SubprocessExecutor(ProcessExecutorBase):
    """Execute prompts via the Aider CLI subprocess.

    Uses ``aider --message`` for non-interactive single-prompt execution.
    Aider operates on git repositories and can edit multiple files
    based on the prompt.
    """
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        """Execute a prompt using the Aider CLI.
        ...
        model : Optional[str]
            Model to use (e.g., "sonnet", "gpt-4o", "deepseek").
        ...
        logger.info(f"{log_prefix} Starting Aider: model={model or 'default'}")
        ...
        try:
            ...
            if not success and stderr_output:
                logger.warning(f"{log_prefix} Aider stderr: {stderr_output[:500]}")
            ...
            return BackendExecutionResult(
                success=False,
                output="",
                error="Aider not found. Install it with: pip install aider-chat",
                duration_ms=int((time.monotonic() - start_time) * 1000),
            )
        except Exception as e:
            logger.error(f"{log_prefix} Aider execution error: {e}")
            return BackendExecutionResult(
                success=False,
                output="",
                error=str(e),
