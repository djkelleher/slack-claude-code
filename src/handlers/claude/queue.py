...
def _build_auto_generated_queue_entries(
    *,
    ...
    task_status : Optional[TaskStatusReport]
        If available, passed through to ``build_continue_prompt`` so the
        continuation prompt can list specific remaining tasks.
    """
    ...
    if should_continue:
        if continue_round < config.QUEUE_AUTO_MAX_CONTINUE_ROUNDS:
            entries.append(
                (
                    build_continue_prompt(task_status=task_status),
                    None,
                    None,
                    None,
                    _build_auto_meta(
...
async def _execute_queue_item(
    ...
    *,
    ...
    processed_queue_item_ids: Optional[list[int]] = None,
    ...
    if trace_lineage_state is None:
        trace_lineage_state = {}
    if processed_queue_item_ids is None:
        processed_queue_item_ids = []
    ...
    if milestone_marker:
        ...
        if trace_service is not None:
            await trace_service.start_explicit_milestone(
    ...
    if (
        ...
        try:
            ...
            if started_trace_run is not None:
                finalized_trace_run = await trace_service.finalize_run_with_status(
            ...
            await deps.db.update_queue_item_status(item.id, "failed", error_message=str(exc))
            processed_queue_item_ids.append(item.id)
            raise
    ...
    try:
        ...
        # Append task-status reporting instructions for auto-each items
        automation_meta_for_suffix = _normalize_automation_meta(
            _result_field(item, "automation_meta", None)
        )
        ...
        if started_trace_run is not None and not trace_run_finalized:
            await trace_service.finalize_run_with_status(
                ...
                output="",
                error="Queue processor cancelled",
                git_tool_events=[],
            )
        await deps.db.update_queue_item_status(
            item.id,
...
async def _run_parallel_group(
    *,
    ...
    session: Session,
    items: list,
    processed_queue_item_ids: list[int],
    ...
    def start_task(queue_item) -> None:
        task = asyncio.create_task(
            _execute_queue_item(
                ...
                override_resume_ids={},
                processed_queue_item_ids=processed_queue_item_ids,
